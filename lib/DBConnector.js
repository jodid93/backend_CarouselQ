...
module.exports.getUserSongs = function(userID, cb){
  pg.connect(DATABASE, function(error,client,done){
    if(error){
      ...
      var values = [userID];
      var query = 'SELECT * from songs where songs_uri = any ((select userinfo_songs from userinfo where userinfo_hashnameid = $1)::text[]);';
...
module.exports.getUsersInQueue = function(queueID, cb){
  pg.connect(DATABASE, function(error,client,done){
    if(error){
      ...
      console.log('náði að tengjast grunni');
      var values = [queueID];
      var query = 'SELECT userinfo_hashnameid, userinfo_name FROM userinfo WHERE userinfo_hashnameid  = any ((select users from queues where queue_id = $1)::text[])';
      client.query(query, values, function(err,  result){
      if(err){
        console.log(err)
        return cb(err)
  ...
  pg.connect(DATABASE, function (error, client, done) {
    ...
    client.query(query, function (err, result) {
      if (err) {
        ...
        var query2 = 'CREATE TABLE if not exists UserInfo( UserInfo_name VARCHAR(100) NOT NULL, UserInfo_songs TEXT[], UserInfo_hashName TEXT[], UserInfo_Qowner boolean, UserInfo_Active boolean, UserInfo_hashNameId VARCHAR(100));';
        client.query(query2, function (err, result) {
          ...
          if (err) {
            ...
            var query3 = 'CREATE TABLE if not exists SONGS( songs_uri VARCHAR(100) NOT NULL, song_name VARCHAR(500), song_band VARCHAR(100), song_skipVotes integer, song_duration integer);';
...
module.exports.addNewUser = function addNewUser(username, queueId, hashedUN, owner, cb){
  pg.connect(DATABASE, function (error, client, done) {
    ...
    var query = 'INSERT INTO userinfo (userinfo_name, userinfo_hashname, UserInfo_Qowner, UserInfo_Active, UserInfo_hashNameId) VALUES($1, $2, $3, true, $4)';
    client.query(query, values, function (err, result) {
